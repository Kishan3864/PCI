          ...
          <Reveal>
            <Card className="corner-frame overflow-hidden text-center">
              <CardContent className="p-10">
                <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-[2px] bg-rose-400/10 text-rose-300 ring-1 ring-inset ring-rose-400/30">
                  <XCircle className="h-7 w-7" />
                </div>
                <h1 className="mt-6 text-2xl font-bold text-white">Scan failed</h1>
  ...
  return (
    <>
      ...
      <div className="mx-auto max-w-3xl px-6 pb-20">
        <Reveal>
          <SpotlightCard className="card-lift corner-frame rounded-[2px] border border-slate-400/15 bg-surface-800/80">
            <Card className="border-transparent bg-transparent shadow-none backdrop-blur-none">
              <CardHeader>
                <CardTitle className="flex items-center gap-2 text-base">
                  <span className="flex h-8 w-8 items-center justify-center rounded-[2px] bg-cyan-400/10 text-cyan-300 ring-1 ring-inset ring-cyan-400/30">
              ...
              <CardContent>
                <ul className="space-y-2 text-sm">
                  {report.securityHeaders.map((h) => (
                    <li
                      key={h.header}
                      className="flex items-center gap-2 rounded-[2px] border border-slate-400/15 bg-surface-900/70 px-3 py-2"
                    >
                      {h.present ? (
                      ...
                      <span className="font-mono text-xs text-slate-200">{h.header}</span>
                      {!h.present ? (
              ...
              {report.externalDomains.length === 0 ? (
                ...
                <ul className="flex flex-wrap gap-2">
                  {report.externalDomains.map((d) => (
                    <li
                      key={d}
                      className="inline-flex items-center gap-2 rounded-[2px] border border-slate-400/15 bg-surface-900/70 px-3 py-1.5 font-mono text-xs text-slate-300"
                    >
        ...
        <Reveal delay={80}>
          <Card className="relative mt-6 overflow-hidden">
            ...
            <CardContent>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-surface-900/70 text-center backdrop-blur-[2px]">
...
function Stat({
  ...
  return (
    <div className="card-lift h-full rounded-[2px] border border-slate-400/15 bg-surface-800/80 p-5 text-center backdrop-blur-sm">
