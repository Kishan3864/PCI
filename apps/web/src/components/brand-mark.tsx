...
/**
 * BrandMark — the ScriptProof radar glyph on a sharp cyan gradient tile.
...
export function BrandMark({ className, glow = true }: { className?: string; glow?: boolean }) {
  return (
    <span
      className={cn(
        'relative flex h-8 w-8 items-center justify-center rounded-[2px] bg-gradient-to-br from-cyan-400 to-blue-600',
        glow && 'shadow-[0_6px_18px_-6px_rgba(34,211,238,0.85)]',
        className,
      )}
    >
      <svg viewBox="0 0 24 24" fill="none" className="h-5 w-5 text-surface-900" aria-hidden>
